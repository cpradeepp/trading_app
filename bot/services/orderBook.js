...
import {getBidAndAskPrice, getOrderBook, getRandomNumber, getVal, generateUuid} from './../helpers/utils';
import {logRefreshData, logTable, logOrders, logInfo, logEndOfInfo} from './../helpers/logger';
...
class OrderBook {
    ...
    placeOrders = async () => {
        ...
        const { bestBid, bestAsk, spread, pendingOrders } = this.tradeData;
        ...
        for(let i = 0; i < ORDER_COUNT; i++) {
            bidOrders.push(this.constructOrderArray(USD, minBid, maxBid, 'bid'));
            askOrders.push(this.constructOrderArray(ETH, minAsk, maxAsk, 'ask'));
        }
    ...
    fulfillOrders = async () => {
        ...
        const { pendingOrders, bestBid, bestAsk, fulfilledOrders } = this.tradeData;
        const bidOrders = pendingOrders.filter( o => o.type === 'bid');
        const askOrders = pendingOrders.filter( o => o.type === 'ask');
     ...
     */
    constructOrderArray = (asset, min, max, type) => {
        ...
        return { id, price, amount, type, created };
    };

    updateTradeDataState = (bidOrders = [], askOrders = [], fulfilledOrders = []) => {
        this.tradeData = {
            ...this.tradeData,
            pendingOrders: [...bidOrders.sort((a, b) => b.price - a.price ), ...askOrders.sort((a, b) => a.price - b.price )],
            ...fulfilledOrders
        };
    };

    updateBalanceAndLog = async (type, order, bestPrice) => {
